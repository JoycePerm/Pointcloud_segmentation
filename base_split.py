...
def cluster_and_save(points, features, eps=0.80, min_samples=10):
    """
    DBSCAN聚类分割底板
    参数:
    points:点云
    ...
    base = all_data[labels == largest_label]
    other = all_data[labels != largest_label]

    return base, other

def expand_base_with_plane_fit(base, other, up_distance_threshold=5.0, down_distance_threshold=-35.0):
    """
    ...
    z_plane = a * x + b * y + c
    # dists = np.abs(z - z_plane)  # 垂直距离
    dists = z - z_plane

    # 筛选距离小于 threshold 的点
    mask_near = (down_distance_threshold < dists) & \
                    (dists < up_distance_threshold)
...
def read_file(input_points, input_features, input_cam_pos):
    """
    读取pcd文件
    参数：
    ...
    cam_pos = np.loadtxt(input_cam_pos,delimiter=' ')

    return points, features, cam_pos


def rotated_points_cam_pos(all_points, cam_pos):
    """
    拟合点云底板并旋转点云
    参数:
        all_points:点云
    ...
    cam_coords = cam_pos[:, 0:3]
    cam_line_idxs = cam_pos[:, 3].astype(int)
    ...
    if np.degrees(np.arccos(np.dot(normal, (0,0,1))))>15:
        rotation_matrix = np.eye(3)
        return all_points, cam_pos, rotation_matrix

    rotated_points_coords = (rotation_matrix @ points_coords.T).T  # shape: (N, 3)
    rotated_cam_coords = (rotation_matrix @ cam_coords.T).T  # shape: (M, 3)
    rotated_points = np.hstack((rotated_points_coords, points_line_idxs.reshape(-1, 1)))
    rotated_cam_pos = np.hstack((rotated_cam_coords, cam_line_idxs.reshape(-1, 1)))

    return rotated_points, rotated_cam_pos, rotation_matrix
...
def split_base_other(rotated_points, all_features):
    ...
    """
    # 划分点云
    grouped_points, grouped_features = chunk_point_feature(rotated_points, all_features)
    ...
    # 根据z轴均值后处理
    all_base, all_other = select_base(all_base, all_other)
...
def process_split_base(input_points, input_features, input_cam_pos):
    """
    主程序-底板分割
    参数:
    ...
    # 读取txt文件
    all_points, all_features, cam_pos = read_file(input_points, input_features, input_cam_pos)

    # 旋转点云和相机轨迹
    rotated_points, rotated_cam_pos, rotation_matrix = rotated_points_cam_pos(all_points, cam_pos)
    ...
    return rotated_base_points, base_features, rotated_other_points, other_features, rotated_cam_pos, rotation_matrix
    ...
    for subdir in os.listdir(input_dir):
        ...
        if subdir_path.is_dir():
            ...
            if pre_downsample_file.exists() and feature_file.exists():
                print("Done")
                start_time = time.time()
                rotated_base_points, base_features, rotated_other_points, other_features, rotated_cam_pos, rotation_matrix = process_split_base(pre_downsample_file,feature_file,camera_file)
                end_time = time.time()
                print(f"底板分割总耗时 {end_time - start_time:.4f} seconds")
                ...
                np.savetxt(out_subdir_path / 'rotated_cam_pos.txt', rotated_cam_pos, fmt='%.3f %.3f %.3f %.3f')
